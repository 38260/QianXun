...
def main():
    ...
    # 启动数据data服务 (端口5001)
    data_thread = threading.Thread(target=run_service, args=("data.py", 5001))
    ...
    # 启动AI分析服务 (端口5003)
    ai_threadGenerator = threading.Thread(target=run_service, args=("generate.py", 5003))
    ai_threadGenerator.daemon = True
    ai_threadGenerator.start()
    
    print("所有服务已启动:")
    print("- 数据API服务: http://localhost:5001")
    print("- AI聊天API服务: http://localhost:5002")
    print("- AI分析服务: http://localhost:5003")
    print("\n按 Ctrl+C 停止所有服务")
